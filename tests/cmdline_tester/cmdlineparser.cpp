...
const QString CmdLineParser::m_helpMessage = QLatin1String(
        ...
        "                 - none [default]\n"
        "                 - full\n"
        "--tabWidth NUM   Number of white spaces used when indentation mode is set to full\n"
...
CmdLineParser::Result CmdLineParser::parse()
{
    ...
    while (m_error.isEmpty() && hasMoreArgs()) {
        ...
        else if (arg == QLatin1String("--tabWidth"))
            handleSetIndentationWidth();
...
void CmdLineParser::handleSetIndentationWidth()
{
    if (hasMoreArgs()) {
        const QString &indentationWidth = nextArg();
        bool ok;
        m_indentationWidth = indentationWidth.toInt(&ok);
        if (!ok)
          m_error = QString(QLatin1String("Unknown tab width '%1'.")).
                                           arg(indentationWidth);
    } else {
        m_error = QLatin1String("Missing tab width.");
    }
}
...
int CmdLineParser::indentationWidth() const
{
